...
basedir = '/home/adriaand/project/actpol/20210326_est_noise_pix'
...
#cov_pix_est[0,0,mask[0]] /= cov_pix[0,0,mask[0]]
#cov_pix_est[1,1,mask[1]] /= cov_pix[1,1,mask[1]]
#cov_pix_est[2,2,mask[2]] /= cov_pix[2,2,mask[2]]
...
# Smooth with small beam.
cov_pix_est_presm = np.zeros_like(cov_pix_est)
...
# Determine mask 
border = scipy.ndimage.distance_transform_edt(mask.reshape(3, minfo.nrow, minfo.nphi[0])) >= 3
border = border.reshape(3, minfo.npix)

for pidx in range(3):
    fig, ax = plt.subplots(dpi=300)
    im = ax.imshow(border[pidx].reshape(minfo.nrow, minfo.nphi[0]), interpolation='none')
    colorbar(im)
    fig.savefig(opj(imgdir, 'border_{}'.format(pidx)))
    plt.close(fig)


border_nxn = np.einsum('il, kl -> ikl', border, border, optimize=True)
cov_pix_est_presm = map_utils.inpaint_nearest(cov_pix_est_presm, border_nxn, minfo)
...
for pidx in range(3):
    for pjdx in range(3):
        fig, ax = plt.subplots(dpi=300)
        im = ax.imshow(cov_pix_est_presm[pidx,pjdx].reshape(minfo.nrow, minfo.nphi[0]), interpolation='none')
        colorbar(im)
        fig.savefig(opj(imgdir, 'cov_est_presm_mask_{}_{}'.format(pidx, pjdx)))
        plt.close(fig)

for pidx in range(3):
    for pjdx in range(3):
        fig, ax = plt.subplots(dpi=300)
        im = ax.imshow(np.log10(np.abs(cov_pix_est_presm[pidx,pjdx].reshape(minfo.nrow, minfo.nphi[0]))), interpolation='none')
        colorbar(im)
        fig.savefig(opj(imgdir, 'cov_est_presm_mask_log_{}_{}'.format(pidx, pjdx)))
        plt.close(fig)

# Add to orig map.
cov_pix_est[~mask_nxn] = cov_pix_est_presm[~mask_nxn]

# Smooth cov_est
cov_pix_est_sm = np.zeros_like(cov_pix_est)
b_ell = hp.gauss_beam(np.radians(5), lmax=lmax)
...
#NOTE
#cov_pix_est_sm[0,0] *= cov_pix[0,0]
#cov_pix_est_sm[1,1] *= cov_pix[1,1]
#cov_pix_est_sm[2,2] *= cov_pix[2,2]


for pidx in range(3):
    for pjdx in range(3):

        if pidx == 0 and pjdx == 0:
            vmin, vmax = 0, 6
        elif pidx == pjdx:
            vmin, vmax = 0, 15
        else:
