...
basedir = '/home/adriaand/project/actpol/20230301_pcg_planck_scaled'
maskdir = '/home/adriaand/project/actpol/20201009_pcg_planck/meta'
imgdir = opj(basedir, 'img_newprec')
...
#fig, axs = plt.subplots(ncols=3, nrows=3, dpi=300, constrained_layout=True)
#for idxs, ax in np.ndenumerate(axs):
#    axs[idxs].plot(isqrt_b_ell[idxs])
#fig.savefig(opj(imgdir, 'isqrt_b_ell'))
#plt.close(fig)
...
#    axs[idxs].plot(icov_ell_scaled[idxs])
#    if idxs[0] == idxs[1]:
...
# NOTE not scaled.
#isqrt_b_ell = mat_utils.matpow(np.ones_like(b_ell), -0.5)
#icov_ell_scaled = np.einsum('abk, bck, cdk -> adk', isqrt_b_ell, icov_ell, isqrt_b_ell)



#b_ell = b_ell * b_ell ** -0.5
#print(b_ell)

#isqrt_beam = operators.EllMatVecAlm(ainfo, isqrt_b_ell)
#icov_scaled = operators.EllMatVecAlm(ainfo, icov_ell_scaled)
#b_ell_scaled = b_ell * mat_utils.matpow(b_ell, -0.5, return_diag=True)

#solver = solvers.CGWienerMap.from_arrays(imap, minfo, ainfo, icov_ell_scaled, icov_pix, draw_constr=True,
#                                         b_ell=b_ell_scaled, mask_pix=mask_I * np.ones(3)[:,np.newaxis],
#                                         spin=[0, 2], swap_bm=False)


solver = solvers.CGWienerMap.from_arrays(imap, minfo, ainfo, icov_ell, icov_pix, draw_constr=True,
                                         b_ell=b_ell, mask_pix=mask_I * np.ones(3)[:,np.newaxis],
                                         spin=[0, 2], swap_bm=False, sfilt=isqrt_b_ell)
...
#     ainfo, icov_ell_scaled, icov_pix_prec, minfo, [0, 2], b_ell=b_ell) #NOTE TRY SCALED b_ell here too! NO more complicated.

# prec_masked_cg = preconditioners.MaskedPreconditionerCG(
#   ainfo, icov_ell_scaled, [0, 2], mask_I[0].astype(bool), minfo, lmax=None, #NOTE
#   nsteps=25, lmax_r_ell=None)

# prec_masked_mg = preconditioners.MaskedPreconditioner(
#   ainfo, icov_ell_scaled[0:1,0:1], 0, mask_I[0].astype(bool), minfo,
#   min_pix=1000, n_jacobi=1, lmax_r_ell=3000) #NOTE

# prec_masked_mg_pol = preconditioners.MaskedPreconditioner(
#   ainfo, icov_ell_scaled, [0, 2], mask_I[0].astype(bool), minfo,
#   min_pix=1000, n_jacobi=1, lmax_r_ell=3000)


prec_pinv = preconditioners.PseudoInvPreconditioner(
    ainfo, icov_ell, icov_pix_prec, minfo, [0, 2], b_ell=b_ell, sfilt=isqrt_b_ell) 

prec_masked_cg = preconditioners.MaskedPreconditionerCG(
  ainfo, icov_ell, [0, 2], mask_I[0].astype(bool), minfo, lmax=None, # NOTE
    nsteps=25, lmax_r_ell=None, sfilt=isqrt_b_ell)

prec_masked_mg = preconditioners.MaskedPreconditioner(
  ainfo, icov_ell[0:1,0:1], 0, mask_I[0].astype(bool), minfo,
    min_pix=1000, n_jacobi=1, lmax_r_ell=3000, sfilt=isqrt_b_ell[0:1,0:1]) # NOTE

#prec_masked_mg_pol = preconditioners.MaskedPreconditioner(
#  ainfo, icov_ell, [0, 2], mask_I[0].astype(bool), minfo,
#    min_pix=1000, n_jacobi=1, lmax_r_ell=3000, sfilt=None)
...
for idx in range(niter_cg):
    ...
    #ps_c_ell[idx,...] = ainfo.alm2cl(isqrt_beam(solver.x)[:,None,:], isqrt_beam(solver.x)[None,:,:])
...
# Plot result
#wiener = np.einsum('abk, bk -> ak', isqrt_b_ell, solver.get_wiener())

#wiener = isqrt_beam(solver.get_wiener())
wiener = solver.get_wiener()
omap = curvedsky.alm2map(wiener, omap)
plot = enplot.plot(omap, colorbar=True, font_size=50, grid=False, range='250:5', downgrade=4)
enplot.write(opj(imgdir, 'alm_out'), plot)

#icov = np.einsum('abk, bk -> ak', icov_ell, solver.get_icov())

#icov = icov_scaled(wiener)
icov = solver.get_icov()
omap = curvedsky.alm2map(icov, omap)
