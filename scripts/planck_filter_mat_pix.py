...
from optweight import sht, map_utils, mat_utils, solvers, operators, preconditioners, cg_pix_filter
...
basedir = '/home/adriaand/project/actpol/20230726_filter_mat_pix'
...
odict = cg_pix_filter(imap, theory_cls, b_ell, lmax,
                      icov_pix=icov_pix, mask_bool=mask_I * np.ones(3)[:,np.newaxis], minfo=minfo,
                      include_te=True, 
                      #niter=15, niter_masked_cg=7, 
                      niter=20, niter_masked_cg=10, 
                      #niter=30, niter_masked_cg=15, 
                      ainfo=ainfo,
                      benchmark=5, verbose=True, err_tol=1e-15, #rtol_icov=1e-2,
                      order=1, lmax_masked_cg=1500, # NOTE, used to be 1300
                      swap_bm=True, scale_a=False) # NOTE
solver = odict['solver']
