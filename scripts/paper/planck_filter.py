...
def main(basedir, draw_constr=False, test_conv=False, niter_cg=20, niter_mg=40,
         ...
         no_beam=False, lmax_masked_cg=1500):
    '''
    ...
    solver = solvers.CGWiener.from_arrays(alm, ainfo, icov_ell, icov_pix, minfo, b_ell=b_ell,
                                          draw_constr=draw_constr, mask_pix=mask_gl,
                                          swap_bm=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-conv', action='store_true',
                        help='Replace b with A(x) for know LCDM x.')
    ...
    main(args.basedir, draw_constr=args.draw_constr, test_conv=args.test_conv,
         ...
         no_beam=args.no_beam, lmax_masked_cg=args.lmax_masked_cg)
